...
if [ "$1" = "prod" ]; then
    ...
    uvicorn qrcode_api:qrcode_app --workers 1 --log-level info --host 0.0.0.0 --port 8080
elif [ "$1" = "dev" ]; then
    echo "Starting Uvicorn server in development mode..."
    # reload implies workers = 1
    uvicorn qrcode_api:qrcode_app --reload --log-level debug --host 0.0.0.0 --port 8080
else
